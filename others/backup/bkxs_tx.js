...
let bkxshd = $.isNode() ? (process.env.bkxshd ? process.env.bkxshd : "sessionid=78d3ab05bb4046f0a8f0ad4ecd04732c") : ($.getdata('bkxshd') ? $.getdata('bkxshd') : "")
let withdraws = $.isNode() ? (process.env.withdraws ? process.env.withdraws : "30") : ($.getdata('withdraws') ? $.getdata('withdraws') : "")
let withdrawNum = process.env.withdrawNum || '20'
let withdrawTime = process.env.withdrawTime || '9:59:59.555'
let allMessage = '';
let bkxshds = ""
const logs = 0;
...
!(async () => {
  ...
  if (!$.isNode()) {
    ...
    if (bkxshd && bkxshd.indexOf('@') > -1) {
      bkxshdArr = bkxshd.split('@');
    ...
    await txTask()
    // if(!await txTask()) return
    console.log(`\n共${bkxshdArr.length}个cookie`)
    for (let k = 0; k < bkxshdArr.length; k++) {
      ...
      console.log(`\n【必看小说TX${$.index}】`)
      //allMessage +=`\n开始【必看小说${$.index}】`
      await withdraw(withdrawResolve(withdraws))
...
async function txTask() {
  if (withdraws) console.log(`当前设置提现金额：${withdraws}`)
  console.log(`当前设置提现时间：${withdrawTime}`)
  console.log(`当前时间：${$.time('HH:mm:ss.S')}`)
  $.withdraw = withdrawResolve(withdraws)

  let [withdrawH, withdrawM, withdrawS] = withdrawTime.split(':')
  $.hour = $.time('HH')
  $.minute = $.time('mm')

  if (withdrawH.length == 1) {
    withdrawH = `0${withdrawH}`
  }
...
function withdrawResolve(withdraw) {
  switch (withdraw) {
    case '1':
      return 1
    case '5':
