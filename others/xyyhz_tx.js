...
withdrawTime = process.env.withdrawTime || '23:59:59.555'
withdrawNum = process.env.withdrawNum || '20'
withdrawAmount = process.env.withdrawAmount || '0.5'
...
async function withdraw(dou) {
  let res = await $.http({
    method: 'post',
    ...taskUrl(`api/account/cash/withdraw`, {
      dou
    })
  })
  res = $.toObj(res)
...
function taskUrl(path, body) {
  let opts = {
    url: `https://x.moonbox.site/${path}`,
    headers: {
      'content-type': 'application/json',
      Cookie: `app-token=${cookie}`,
